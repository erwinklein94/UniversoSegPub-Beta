/* =======================================================
   ...
   Objetivo: proteger mobile/desktop contra sobreposição, duplicidade
   de blocos antigos e estouro horizontal depois das customizações.
   ======================================================= */
...
(function revisarEstabilidadeGeralDoSite() {
  function inserirCss() {
    ...
    style.textContent = `
      ...
      .prf-dados-publicos-card,
      .sidebar-products-unified,
      .comparador-check-grupo,
      .produto-card,
      .remuneracao-card,
      .card {
        max-width: 100% !important;
      ...
      .sidebar .sidebar-extra-products,
      .sidebar .sidebar-products-cta,
      .sidebar .sidebar-ad,
      .sidebar .ad-slot--sidebar {
        max-width: 100% !important;
      }

      body[data-page="produtos"] #page-produtos .produto-card,
      .page-produtos #page-produtos .produto-card {
      ...
      @media (max-width: 760px) {
        ...
        .sidebar-products-unified-card,
        .comparador-check-option,
        .produto-card {
          min-width: 0 !important;
        }
      }
  ...
  function removerDuplicidadesAntigasDaSidebar() {
    ...
    if (!sidebar) return;
    const vitrineUnica = sidebar.querySelector('.sidebar-products-unified');
    if (!vitrineUnica) return;

    sidebar.querySelectorAll('.sidebar-extra-products, .sidebar-products-cta, .sidebar-ad, .ad-slot--sidebar').forEach((el) => {
      if (el.closest('.sidebar-products-unified')) return;
      el.remove();
    });
  }
  ...
  document.addEventListener('change', () => window.setTimeout(aplicar, 60), true);
  const observer = new MutationObserver(() => window.setTimeout(aplicar, 50));
